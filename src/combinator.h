            ...
            ) 
            {
              ...
              for(ItA a = a_begin; a!=a_end; ++a)
              {
                ...
                */
                ...
                for(const typename CombinationList::iterator pair_list_itr=local_result.begin(); pair_list_itr!=local_result.end();++pair_list_itr)
                {
                  typename CombinationList::value_type & pair_list = * pair_list_itr;
    ...
    inline CombinationList make_unique_pairs_impl(It it_begin, It it_end)
    {
      ...
      */
        ...
        inline CombinationList make_unique_pairs(It it_begin, It it_end) 
        {
          ...
          if( distance % 2 == 0) { //even size: just call algorithm
            ...
            for(typename CombPtr::iterator it = tmp_result_with_nullptr_pairs.begin(); it!=tmp_result_with_nullptr_pairs.end(); ++it)
            {
              ...
              for(const typename Pairs_t::iterator p = ptr_pair_list.begin();  p!=ptr_pair_list.end(); ++p) 
      ...
      template< class Ps>
        ...
        {
          for( const typename Ps::iterator pitr = P.begin();pitr!=P.end(); ++pitr)
            //for( const auto & p: P)
          {
            typename Ps::value_type & p = *pitr;
      ...
      template< class Cs>
        ...
        {
          int idx=1;
          for( const typename  Cs::iterator it = C.begin();it!=C.end(); ++it)// for(const auto & P: C)
          {
            typename Cs::value_type & P = *it;
      ...
      template<class Cont>
        ...
        {
          std::cout << name << " = {";
          for(const typename Cont::iterator it = A.begin(); it!=A.end(); ++it)
          {
            std::cout << *it;
